...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class AircraftTypeRefCombo : UserControl {
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeAircraftTypeRefServiceClient aircraftTypeRef = null;

                try {
                    aircraftTypeRef = new CrudeAircraftTypeRefServiceClient();
                    List<CrudeAircraftTypeRefContract> contracts = aircraftTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "AircraftTypeName";
                    cboRef.ValueMember = "AircraftTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (aircraftTypeRef != null) aircraftTypeRef.Close();
                }
            }
        }
    }
