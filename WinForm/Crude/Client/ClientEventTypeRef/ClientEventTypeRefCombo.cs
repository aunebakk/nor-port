...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class ClientEventTypeRefCombo : UserControl {
        ...
        public ClientEventTypeRefCombo() {
           InitializeComponent();
        }
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeClientEventTypeRefServiceClient clientEventTypeRef = null;

                try {
                    clientEventTypeRef = new CrudeClientEventTypeRefServiceClient();
                    List<CrudeClientEventTypeRefContract> contracts = clientEventTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "ClientEventTypeName";
                    cboRef.ValueMember = "ClientEventTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (clientEventTypeRef != null) clientEventTypeRef.Close();
                }
            }
        }
    }
