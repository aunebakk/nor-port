...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class AddressTypeRefCombo : UserControl {
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeAddressTypeRefServiceClient addressTypeRef = null;

                try {
                    addressTypeRef = new CrudeAddressTypeRefServiceClient();
                    List<CrudeAddressTypeRefContract> contracts = addressTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "AddressTypeName";
                    cboRef.ValueMember = "AddressTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (addressTypeRef != null) addressTypeRef.Close();
                }
            }
        }
    }
