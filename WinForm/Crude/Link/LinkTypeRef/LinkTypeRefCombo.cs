...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class LinkTypeRefCombo : UserControl {
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeLinkTypeRefServiceClient linkTypeRef = null;

                try {
                    linkTypeRef = new CrudeLinkTypeRefServiceClient();
                    List<CrudeLinkTypeRefContract> contracts = linkTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "LinkTypeName";
                    cboRef.ValueMember = "LinkTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (linkTypeRef != null) linkTypeRef.Close();
                }
            }
        }
    }
