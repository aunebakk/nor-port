...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class ContactMethodRefCombo : UserControl {
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeContactMethodRefServiceClient contactMethodRef = null;

                try {
                    contactMethodRef = new CrudeContactMethodRefServiceClient();
                    List<CrudeContactMethodRefContract> contracts = contactMethodRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "ContactMethodName";
                    cboRef.ValueMember = "ContactMethodRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (contactMethodRef != null) contactMethodRef.Close();
                }
            }
        }
    }
