...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class DefaultUserActivityTypeRefCombo : UserControl {
        ...
        public DefaultUserActivityTypeRefCombo() {
           InitializeComponent();
        }
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeDefaultUserActivityTypeRefServiceClient defaultUserActivityTypeRef = null;

                try {
                    defaultUserActivityTypeRef = new CrudeDefaultUserActivityTypeRefServiceClient();
                    List<CrudeDefaultUserActivityTypeRefContract> contracts = defaultUserActivityTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "DefaultUserActivityTypeName";
                    cboRef.ValueMember = "DefaultUserActivityTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (defaultUserActivityTypeRef != null) defaultUserActivityTypeRef.Close();
                }
            }
        }
    }
