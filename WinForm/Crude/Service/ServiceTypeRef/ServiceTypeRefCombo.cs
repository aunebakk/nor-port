...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class ServiceTypeRefCombo : UserControl {
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeServiceTypeRefServiceClient serviceTypeRef = null;

                try {
                    serviceTypeRef = new CrudeServiceTypeRefServiceClient();
                    List<CrudeServiceTypeRefContract> contracts = serviceTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "ServiceTypeName";
                    cboRef.ValueMember = "ServiceTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (serviceTypeRef != null) serviceTypeRef.Close();
                }
            }
        }
    }
