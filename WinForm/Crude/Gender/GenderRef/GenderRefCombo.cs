...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class GenderRefCombo : UserControl {
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeGenderRefServiceClient genderRef = null;

                try {
                    genderRef = new CrudeGenderRefServiceClient();
                    List<CrudeGenderRefContract> contracts = genderRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "GenderName";
                    cboRef.ValueMember = "GenderRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (genderRef != null) genderRef.Close();
                }
            }
        }
    }
