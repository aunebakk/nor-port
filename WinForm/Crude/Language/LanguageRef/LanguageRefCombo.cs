...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class LanguageRefCombo : UserControl {
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeLanguageRefServiceClient languageRef = null;

                try {
                    languageRef = new CrudeLanguageRefServiceClient();
                    List<CrudeLanguageRefContract> contracts = languageRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "LanguageName";
                    cboRef.ValueMember = "LanguageRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (languageRef != null) languageRef.Close();
                }
            }
        }
    }
