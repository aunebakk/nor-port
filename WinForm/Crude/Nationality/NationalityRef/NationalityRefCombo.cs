...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class NationalityRefCombo : UserControl {
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeNationalityRefServiceClient nationalityRef = null;

                try {
                    nationalityRef = new CrudeNationalityRefServiceClient();
                    List<CrudeNationalityRefContract> contracts = nationalityRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "NationalityName";
                    cboRef.ValueMember = "NationalityRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (nationalityRef != null) nationalityRef.Close();
                }
            }
        }
    }
