...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class BookingPassengerEventTypeRefCombo : UserControl {
        ...
        public BookingPassengerEventTypeRefCombo() {
           InitializeComponent();
        }
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeBookingPassengerEventTypeRefServiceClient bookingPassengerEventTypeRef = null;

                try {
                    bookingPassengerEventTypeRef = new CrudeBookingPassengerEventTypeRefServiceClient();
                    List<CrudeBookingPassengerEventTypeRefContract> contracts = bookingPassengerEventTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "BookingPassengerEventTypeName";
                    cboRef.ValueMember = "BookingPassengerEventTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (bookingPassengerEventTypeRef != null) bookingPassengerEventTypeRef.Close();
                }
            }
        }
    }
