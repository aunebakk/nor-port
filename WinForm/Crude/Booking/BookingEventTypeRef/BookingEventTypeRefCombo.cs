...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class BookingEventTypeRefCombo : UserControl {
        ...
        public BookingEventTypeRefCombo() {
           InitializeComponent();
        }
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeBookingEventTypeRefServiceClient bookingEventTypeRef = null;

                try {
                    bookingEventTypeRef = new CrudeBookingEventTypeRefServiceClient();
                    List<CrudeBookingEventTypeRefContract> contracts = bookingEventTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "BookingEventTypeName";
                    cboRef.ValueMember = "BookingEventTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (bookingEventTypeRef != null) bookingEventTypeRef.Close();
                }
            }
        }
    }
