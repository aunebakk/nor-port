...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class TitleRefCombo : UserControl {
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeTitleRefServiceClient titleRef = null;

                try {
                    titleRef = new CrudeTitleRefServiceClient();
                    List<CrudeTitleRefContract> contracts = titleRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "TitleName";
                    cboRef.ValueMember = "TitleRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (titleRef != null) titleRef.Close();
                }
            }
        }
    }
