...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class FinancialTaxTypeRefCombo : UserControl {
        ...
        public FinancialTaxTypeRefCombo() {
           InitializeComponent();
        }
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeFinancialTaxTypeRefServiceClient financialTaxTypeRef = null;

                try {
                    financialTaxTypeRef = new CrudeFinancialTaxTypeRefServiceClient();
                    List<CrudeFinancialTaxTypeRefContract> contracts = financialTaxTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "FinancialTaxTypeName";
                    cboRef.ValueMember = "FinancialTaxTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (financialTaxTypeRef != null) financialTaxTypeRef.Close();
                }
            }
        }
    }
