...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class FinancialFareTypeRefCombo : UserControl {
        ...
        public FinancialFareTypeRefCombo() {
           InitializeComponent();
        }
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeFinancialFareTypeRefServiceClient financialFareTypeRef = null;

                try {
                    financialFareTypeRef = new CrudeFinancialFareTypeRefServiceClient();
                    List<CrudeFinancialFareTypeRefContract> contracts = financialFareTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "FinancialFareTypeName";
                    cboRef.ValueMember = "FinancialFareTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (financialFareTypeRef != null) financialFareTypeRef.Close();
                }
            }
        }
    }
