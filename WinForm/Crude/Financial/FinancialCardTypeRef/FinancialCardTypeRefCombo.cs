...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class FinancialCardTypeRefCombo : UserControl {
        ...
        public FinancialCardTypeRefCombo() {
           InitializeComponent();
        }
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeFinancialCardTypeRefServiceClient financialCardTypeRef = null;

                try {
                    financialCardTypeRef = new CrudeFinancialCardTypeRefServiceClient();
                    List<CrudeFinancialCardTypeRefContract> contracts = financialCardTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "FinancialCardTypeName";
                    cboRef.ValueMember = "FinancialCardTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (financialCardTypeRef != null) financialCardTypeRef.Close();
                }
            }
        }
    }
