...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class FinancialBookingTransactionTypeRefCombo : UserControl {
        ...
        public FinancialBookingTransactionTypeRefCombo() {
           InitializeComponent();
        }
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeFinancialBookingTransactionTypeRefServiceClient financialBookingTransactionTypeRef = null;

                try {
                    financialBookingTransactionTypeRef = new CrudeFinancialBookingTransactionTypeRefServiceClient();
                    List<CrudeFinancialBookingTransactionTypeRefContract> contracts = financialBookingTransactionTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "FinancialBookingTransactionTypeName";
                    cboRef.ValueMember = "FinancialBookingTransactionTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (financialBookingTransactionTypeRef != null) financialBookingTransactionTypeRef.Close();
                }
            }
        }
    }
