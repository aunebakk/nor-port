...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class FinancialPaymentTypeRefCombo : UserControl {
        ...
        public FinancialPaymentTypeRefCombo() {
           InitializeComponent();
        }
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeFinancialPaymentTypeRefServiceClient financialPaymentTypeRef = null;

                try {
                    financialPaymentTypeRef = new CrudeFinancialPaymentTypeRefServiceClient();
                    List<CrudeFinancialPaymentTypeRefContract> contracts = financialPaymentTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "FinancialPaymentTypeName";
                    cboRef.ValueMember = "FinancialPaymentTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (financialPaymentTypeRef != null) financialPaymentTypeRef.Close();
                }
            }
        }
    }
