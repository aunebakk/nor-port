...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class AirlineTypeRefCombo : UserControl {
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeAirlineTypeRefServiceClient airlineTypeRef = null;

                try {
                    airlineTypeRef = new CrudeAirlineTypeRefServiceClient();
                    List<CrudeAirlineTypeRefContract> contracts = airlineTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "AirlineTypeName";
                    cboRef.ValueMember = "AirlineTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (airlineTypeRef != null) airlineTypeRef.Close();
                }
            }
        }
    }
