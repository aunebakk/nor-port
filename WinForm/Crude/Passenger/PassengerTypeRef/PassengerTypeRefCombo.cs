...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class PassengerTypeRefCombo : UserControl {
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudePassengerTypeRefServiceClient passengerTypeRef = null;

                try {
                    passengerTypeRef = new CrudePassengerTypeRefServiceClient();
                    List<CrudePassengerTypeRefContract> contracts = passengerTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "PassengerTypeName";
                    cboRef.ValueMember = "PassengerTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (passengerTypeRef != null) passengerTypeRef.Close();
                }
            }
        }
    }
