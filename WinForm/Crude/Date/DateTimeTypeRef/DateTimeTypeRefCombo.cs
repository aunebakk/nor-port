...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class DateTimeTypeRefCombo : UserControl {
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeDateTimeTypeRefServiceClient dateTimeTypeRef = null;

                try {
                    dateTimeTypeRef = new CrudeDateTimeTypeRefServiceClient();
                    List<CrudeDateTimeTypeRefContract> contracts = dateTimeTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "DateTimeTypeName";
                    cboRef.ValueMember = "DateTimeTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (dateTimeTypeRef != null) dateTimeTypeRef.Close();
                }
            }
        }
    }
