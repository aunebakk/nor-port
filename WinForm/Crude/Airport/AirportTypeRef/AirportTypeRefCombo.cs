...
namespace SolutionNorSolutionPort.UserInterface {
    ...
    public partial class AirportTypeRefCombo : UserControl {
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeAirportTypeRefServiceClient airportTypeRef = null;

                try {
                    airportTypeRef = new CrudeAirportTypeRefServiceClient();
                    List<CrudeAirportTypeRefContract> contracts = airportTypeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "AirportTypeName";
                    cboRef.ValueMember = "AirportTypeRcd";
                } catch (Exception ex) {
                    if (ex != null) { }
                } finally {
                    if (airportTypeRef != null) airportTypeRef.Close();
                }
            }
        }
    }
