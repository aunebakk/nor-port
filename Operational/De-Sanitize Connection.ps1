...
# SQL2X Version: 1.0
# http://sql2x.org/
# Generated Date: 12/31/2020 11:21:34 AM
...
if (-Not $PSBoundParameters.ContainsKey('connectionStringSQLServer')) {
    if ($toSQLServerLocalTrusted) {
        $connectionStringSQLServer = "Data Source=(localdb)\MSSQLLocalDB;Integrated Security=True;Connect Timeout=30;Encrypt=False;TrustServerCertificate=True;ApplicationIntent=ReadWrite;MultiSubnetFailover=False;Initial Catalog=NorSolutionPort;"
        ...
        if ($toAzure) {
            $connectionStringSQLServer = "Data Source=$sqlServerName.database.windows.net;Persist Security Info=True;User ID=$sqlServerUserName;Password=$sqlServerPassword;Pooling=False;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=120;Initial Catalog=NorSolutionPort;"
        } else {
            $connectionStringSQLServer = "Server=$sqlServerName;User Id=$sqlServerUserName;Password=$sqlServerPassword;Initial Catalog=NorSolutionPort;"
        }
    }
