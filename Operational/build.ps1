...
param (
    ...
    [string]$zenCopyPackagesFrom = "SolutionNorSolutionPort",
...
try {
    ...
    if ($answer -ne 'no' -and ($doDevelopment -or $doTest)) { 
        ...
        $psi.WorkingDirectory = $sql2x + '\' + 'SolutionNorSolutionPort'
        $psi.UseShellExecute = $false 
        $psi.RedirectStandardOutput = $true 
        $psi.RedirectStandardError = $true 
        $psi.FileName = $vs2017Build
        $psi.Arguments = @($sql2x + '\' + 'SolutionNorSolutionPort' + '\' + 'SolutionNorSolutionPort' + '.sln' + ' /target:clean') 
...
try {
    ...
    if ($answer -ne 'no' -and ($doDevelopment -or $doTest)) { 
        ...
        $psi.RedirectStandardError = $true 
        $psi.FileName = $vs2017Build
        $psi.Arguments = @($sql2x + '\' + 'SolutionNorSolutionPort' + '\' + 'SolutionNorSolutionPort' + '.sln' + ' /target:restore') 
...
try {
    ...
    if ($answer -ne 'no' -and ($doDevelopment -or $doTest)) { 
        ...
        $psi.RedirectStandardError = $true 
        $psi.FileName = $vs2017Build
        $psi.Arguments = @($sql2x + '\' + 'SolutionNorSolutionPort' + '\' + 'SolutionNorSolutionPort' + '.sln' + ' /target:build') 
